...
def main():
    ...
    query_path = os.path.join("tests", "6.png")
    if not os.path.exists(query_path): 
        print(f"Missing 6.png")
        return
    ...
    for mask_name in ["1-dot", "3-dot", "5-dot"]:
        ...
        for filename, ref_img in reference_database:
            ...
            matches.append((filename, ncc_score, ref_img))
            
        matches.sort(key=lambda x: x[1], reverse=True)
        top_15 = matches[:15]
        
        penalized_matches = []
        for filename, ncc_score, ref_img in top_15:
            color_dist = compute_color_distance(warped, ref_img, mask_np)
            penalized = ncc_score - (color_dist * 0.2)
            penalized_matches.append((filename, penalized, ncc_score, color_dist))
            
        penalized_matches.sort(key=lambda x: x[1], reverse=True)
        
        print(f"\n==========================================")
        print(f"Test File: 6.png ({mask_name})")
        print(f"==========================================")
        for i in range(10):
            print(f"  {i+1}. {penalized_matches[i][0]:<40} | Final: {penalized_matches[i][1]:.4f} | NCC: {penalized_matches[i][2]:.4f} | HSV Dist: {penalized_matches[i][3]:.4f}")
